      ...
      <div class="flex items-start justify-between bg-gray-50 p-4 rounded-lg">
        <div>
          ...
          <p class="font-semibold"
            :class="{{ $review->orderDetail->order->user ? '\'text-gray-800\'' : '\'italic text-gray-500\'' }}">
            {{ $review->orderDetail->order->user->name ?? 'Deleted User' }}
          </p>

          {{-- product name --}}
          <p class="text-sm"
            :class="{{ $review->orderDetail->product ? '\'text-pink-600\'' : '\'italic text-gray-500\'' }}">
            {{ $review->orderDetail->product->name ?? 'Deleted Product' }}
          </p>
